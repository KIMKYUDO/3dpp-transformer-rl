...
class PackingEnv:
    ...
    def _orientations(l: int, w: int, h: int) -> List[Box]:
        """6개 회전 반환 (중복 포함, Policy와 인덱스 일치 보장)
        
        Policy의 OrientationEmbedder와 동일한 순서:
        0: (l,w,h), 1: (l,h,w), 2: (w,l,h),
        3: (w,h,l), 4: (h,l,w), 5: (h,w,l)
        
        중복 제거 안 함! (20,10,10)이면 [0,1,4]가 같아도 6개 모두 반환
        """
        return [
            (l, w, h),  # 0
            (l, h, w),  # 1
    ...
    def step(self, action: Tuple[int, int, int, int]):
        ...
        if not (0 <= bidx < len(self.boxes)) or self.used[bidx]:
            ...
            done = (self.invalid_count >= self.cfg.max_invalid_attempts)
            info["terminated_by"] = "too_many_invalid" if done else None
            info["invalid_count"] = self.invalid_count

            return obs, float(self.cfg.invalid_penalty), done, info

        # ✅ 회전(orientation) 적용
        orientations = self._orientations(*self.boxes[bidx])
        
        # oidx 범위 체크 (항상 6개이므로 0~5)
        if not (0 <= oidx < 6):
            self.invalid_count += 1
            obs = {"height": self.height.copy(), 
                   "remaining": (~self.used).sum(), 
                   "gap": self._gap()}
            info["reason"] = "invalid_orientation"
            info["detail"] = f"oidx={oidx}, expected 0~5"
        ...
        l, w, h = orientations[oidx]

        if not self._within_bounds(x, y, l, w):
            self.invalid_count += 1
            obs = {"height": self.height.copy(), 
                   "remaining": (~self.used).sum(), 
                   "gap": self._gap()}
            info["reason"] = "out_of_bounds"
            info["detail"] = f"xy=({x},{y}), size=({l},{w})"
        ...
        self._raise_region(x, y, l, w, top, h)
        self.placed_boxes.append((x, y, top, l, w, h))
        self.used[bidx] = True
        self._sum_vol += int(l * w * h)
